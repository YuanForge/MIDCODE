...
function MiniTrendChart({ points }: { points: AdminTrendPoint[] }) {
  ...
  const path = points
    ...
    .join(' ')
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="h-full w-full overflow-visible text-primary" preserveAspectRatio="none">
...
function TrendSection() {
  ...
  const maxVal = data.points.length > 0 ? Math.max(...data.points.map((p) => p.value)) : 0
  ...
  return (
          ...
          <div className="flex rounded-md border overflow-hidden text-xs">
            {(Object.keys(DIM_LABELS) as TrendDim[]).map((d) => (
              <button
                ...
                aria-pressed={dim === d}
                onClick={() => setDim(d)}
                className={`h-10 px-3 transition-colors ${dim === d ? 'bg-foreground text-background font-medium' : 'text-muted-foreground hover:bg-muted focus-visible:bg-muted'}`}
              >
                {DIM_LABELS[d]}
              </button>
            ))}
          ...
          <div className="flex rounded-md border overflow-hidden text-xs">
            {([7, 30] as TrendDays[]).map((d) => (
              <button
                ...
                aria-pressed={days === d}
                onClick={() => setDays(d)}
                className={`h-10 px-3 transition-colors ${days === d ? 'bg-foreground text-background font-medium' : 'text-muted-foreground hover:bg-muted focus-visible:bg-muted'}`}
              >
                近{d}天
              </button>
            ))}
    ...
    >
        {loading ? (
          ...
          <div className="flex h-32 items-center justify-center text-sm text-muted-foreground">暂无趋势数据</div>
        ) : (
          <div className="h-32 w-full">
            <MiniTrendChart points={data.points} />
          </div>
        )}
        {/* x 轴标签：每隔 N 天显示一个 */}
        ...
        {!loading && !error && data.points.length > 0 ? (
          <div className="mt-3 flex items-center justify-between border-t pt-3">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-semibold tabular-nums">{formatVal(maxVal)}</p>
            </div>
          </div>
        ) : null}
    </PageSection>
        ...
        <div className="flex rounded-md border overflow-hidden text-xs">
          {(Object.keys(tabLabels) as Array<'users' | 'models' | 'channels'>).map((t) => (
            <button
              ...
              aria-pressed={tab === t}
              onClick={() => setTab(t)}
              className={`h-10 px-3 transition-colors ${tab === t ? 'bg-foreground text-background font-medium' : 'text-muted-foreground hover:bg-muted focus-visible:bg-muted'}`}
            >
              {tabLabels[t]}
            </button>
          ))}
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className="h-8 w-32" /> : <p className="text-2xl font-semibold tabular-nums">¥{fmtCredits(stats.total?.revenue)}</p>}
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className="h-8 w-32" /> : <p className="text-2xl font-semibold tabular-nums">¥{fmtCredits(stats.total?.cost)}</p>}
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className="h-8 w-32" /> : (
              <p className={`text-2xl font-semibold tabular-nums ${profitColor(stats.total?.profit)}`}>
      ...
      {marginPct !== null ? (
        <Card>
          ...
          <CardContent>
            <p className={`text-3xl font-semibold tabular-nums ${profitColor(parseFloat(marginPct))}`}>
              {marginPct}%
            </p>
