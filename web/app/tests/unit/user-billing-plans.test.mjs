...
test('billing owns card purchase, redemption, history, and balance refresh', async () => {
  ...
  const redemptionIndex = billingPage.indexOf('<CardRedemptionSection')
  const modelCreditsIndex = billingPage.indexOf('title="专属模型积分"')
  const plansIndex = billingPage.indexOf('title="选择充值套餐"')
  assert.ok(redemptionIndex !== -1)
  assert.ok(modelCreditsIndex !== -1)
  assert.ok(plansIndex !== -1)
  assert.ok(modelCreditsIndex < redemptionIndex)
  ...
  assert.match(redemptionSection, /if\s*\(\s*!trimmedCode\s*\|\|\s*submittingRef\.current\s*\)\s*return/)
