...
服务首次启动且数据库中不存在管理员时，会创建一个 bootstrap 管理员：

- 默认邮箱：`admin@local.invalid`
- 默认用户名：`admin`
- 密码来源：优先使用 `FANAPI_BOOTSTRAP_ADMIN_PASSWORD`；未设置时启动日志会生成并打印一次随机密码

> **请保存首次启动日志中的随机密码，并在首次登录后立即修改管理员信息。**
